...
extern const uint8_t index_html_start[] asm("_binary_index_html_start");// address strings
extern const uint8_t index_html_end[] asm("_binary_index_html_end"); // do lon cua chuoi

static http_post_callback_t http_post_switch_callback = NULL;
static http_post_callback_t http_post_slider_callback = NULL;
static http_get_callback_t http_get_bme280_callback = NULL;
...
static const httpd_uri_t get_bme280 = {
    .uri = "/bme280",
    .method = HTTP_GET,
...
void bme280_response(char *data, int len)
{
    ...
    /* An HTTP GET handler */
static esp_err_t bme280_get_handler(httpd_req_t *req)
{
    // const char *resp_str = (const char *)"{\"temperature\": \"27.3\",\"humidity\": \"80\"}";
    // httpd_resp_send(req, resp_str, strlen(resp_str));
    REQ = req;
    http_get_bme280_callback();
    return ESP_OK;
}

static const httpd_uri_t get_data_bme280 = {
    .uri = "/getDataBme280",
    .method = HTTP_GET,
    .handler = bme280_get_handler,
    /* Let's pass response string in user
     * context to demonstrate it's usage */
    .user_ctx = NULL};
...
{
    if (strcmp("/bme280", req->uri) == 0)
    {
        httpd_resp_send_err(req, HTTPD_404_NOT_FOUND, "/bme280 URI is not available");
...
void start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(server, &get_bme280);
        httpd_register_uri_handler(server, &get_data_bme280);
